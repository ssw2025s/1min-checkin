...
class OneMinAutoCheckin {
  constructor(account) {
    ...
    this.totpSecret = this.validateTotpSecret(account.totp || account.totp_secret);
    this.deviceId = this.generateDeviceId();
  }

  validateTotpSecret(secret) {
    return secret && secret !== "null" && String(secret).trim() !== "" ? secret : null;
  }
  ...
  async login() {
    ...
    const response = await this.fetchWithTimeout("https://api.1min.ai/auth/login", {
      method: "POST",
  ...
  async performMFAVerification(tempToken) {
    ...
    const response = await this.fetchWithTimeout("https://api.1min.ai/auth/mfa/verify", {
      method: "POST",
      headers: this.buildHeaders(),
  ...
  async displayCreditInfo(responseData) {
    ...
    if (!user?.teams || user.teams.length === 0) {
      ...
      return { userName: this.maskEmail(this.email), finalCredit: 0, creditDiff: 0 };
    }
    ...
    let targetTeam = user.teams.find(
      (t) => t.team?.subscription?.userId === userUuid
    ) || user.teams[0];
    ...
    return await this.fetchLatestCredit(teamId, authToken, userName, usedCredit, initialCredit);
  }
  ...
  async fetchLatestCredit(teamId, authToken, userName, usedCredit, initialCredit = 0) {
    ...
    const currentCredit =
      initialCredit > 0 ? initialCredit : await this.getCredits(teamId, headers);
  ...
  async run() {
    try {
      ...
      return { success: false, email: this.maskEmail(this.email), error: error.message };
    }
  }
}
...
async function main() {
  ...
  let accounts = [];
  const accountsJson = process.env.ACCOUNTS_JSON || core.getInput("accounts_json");
